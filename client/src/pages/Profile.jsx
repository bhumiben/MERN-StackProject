...
const handleShowPostings = async () => {
  try{
    ...
    const res = await fetch(`/api/user/postings/${currentUser._id}`);
    const data = await res.json();
    if (data.success === false){
...
const handlePostingDelete = async (postingId) => {
  try {
    const res = await fetch(`/api/posting/delete/${postingId}`,{
      method: 'DELETE',
    });
    const data = await res.json();
    if (data.success === false){
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit}className='flex flex-col gap-4'>
        ...
        hover:opacity-95'to={"/create-posting"}>
          Create Post
        </Link>
      </form>
    ...
    <button onClick={handleShowPostings} className='text-green-700 w-full'>
      Show Posts
    </button>
    <p className='text-red-700 mt-5'>{showPostingsError ?
    'Error showing postings' : ''}</p>
    {userPostings && userPostings.length > 0 &&
    <div className='flex flex-col gap-4'>
      <h1 className='text-center mt-7 text-2xl font-semibold'>Your Posts</h1>
    {userPostings.map((posting) => (
      <div key={posting._id} className='border rounded-lg p-3 flex justify-between items-center gap-4'> 
      <Link to={`/posting/${posting._id}`}>
        <img src={posting.imageUrls[0]} alt="posting cover"
        className='h-16 w-16 object-contain '/>
      </Link>
      <Link  className="text-slate-700 font-semibold 
        hover:underline truncate flex-1"to={`/posting/${posting._id}`}>
        <p>{posting.description}</p>
      </Link>
      <div className='flex flex-col item-center'>
        <button onClick={()=>handlePostingDelete(posting._id)}className='text-red-700 uppercase'>Delete</button>
        <Link to={`/update-posting/${posting._id}`}>
